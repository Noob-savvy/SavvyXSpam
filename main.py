...
def load_plugins(plugin_name):
    path = Path(f"SavvyBots/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"SavvyBots.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["SavvyBots.modules." + plugin_name] = load
    print("Altron has Imported " + plugin_name)


files = glob.glob("SavvyBots/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\n𝐒𝐀𝐕𝐕𝐘 𝐁𝐎𝐓𝐒 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 ⚡\nMy Master ---> @𝐍𝐎𝐎𝐁_𝐒𝐀𝐕𝐕𝐘")
